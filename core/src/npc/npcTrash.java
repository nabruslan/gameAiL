...
public class npcTrash implements NPC {
	...
	public npcTrash(final AiL game, final Screens screen) {//конструктор	
		this.game = game;//инициализация объекта класса игры	
		this.screen = screen;//инициализация сцены
		
		npcImage = new Texture(Gdx.files.internal("trash.png"));	//загрузка изображений	
	...
	public void setText() {
		answers.add("Мусорка: <...>");
		answers.add("Мусорка: <там пусто>");
		answers.add("Мусорка: <все кидают мусор куда попало>");
		answers.add("Мусорка: <разговариваю с мусоркой, мда...>");
		questions.add("1. <заглянуть>");
		questions.add("2. <осмотреть>");
		questions.add("3. Как дела, мусорка?");
		message.setMessage("Мусорка: <там все-таки не пусто, там была монетка>");
	}
	...
	public void getItem(Items item) {//получение предмета
		if(item.getName() == "itemApple") {//если нужный предмет
			...
			message.setMessage("Мусорка: Это мне не нужно...");//установить сообщение
			showMessage = true;//показать сообщение
