 ...
 * Пример класса неигрового персонажа, реализующий интерфейс NPC*/

public class npcDana implements NPC {
	...
	public npcDana(final AiL game, final Screens screen) {//конструктор	
		this.game = game;//инициализация объекта класса игры	
		this.screen = screen;//инициализация сцены
		
		npcImage = new Texture(Gdx.files.internal("Dana.png"));	//загрузка изображений	
		talkAnimation = new Animator(4, 2, "danaAnimation.png", game);
	...
	public void draw() {//отрисовка НПЦ
		...
		else talkAnimation.draw(npcRectangle.x - 30, npcRectangle.y - 10,
								npcRectangle.width + 70, npcRectangle.height + 60, false);
	}
	...
	@Override
	public void setText() {
		answers.add("Дана: Привет!");
		answers.add("Дана: Пишу диплом, ничего не успеваю.");
		answers.add("Дана: Хочу распечатать экономическое обснование, но не могу найти флешку.");
		answers.add("Дана: Я целыми днями пою и репетирую со своей группой");
		questions.add("1. Как у тебя дела?");
		questions.add("2. Что ты тут делаешь?");
		questions.add("3. Что ты делаешь в свободное время?");
		message.setMessage("Дана: Спасибо, что вернул мне флеху!  Держи эту странную бумажку");//установить текст сообщения
	}
	...
	public void getItem(Items item) {//получение предмета
		if(item.getName() == "itemFlash") {//если нужный предмет
			talkAnimation.reset();
			getItem = true;
			...
			for(int i = 0; i < itemList.size(); i++)
				if(itemList.get(i).getName() == "itemPassword") {
					itemList.get(i).update();
				}
		}
		else {//иначе
			talkAnimation.reset();
			message.setMessage("Дана: Это мне не нужно...");//установить сообщение
			showMessage = true;//показать сообщение
