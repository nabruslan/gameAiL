...
public class npcKatya implements NPC {
	...
	public npcKatya(final AiL game, final Screens screen) {//конструктор	
		this.game = game;//инициализация объекта класса игры	
		this.screen = screen;//инициализация сцены
		
		npcImage = new Texture(Gdx.files.internal("Katya.png"));	//загрузка изображений	
	...
	@Override
	public void setText() {
		answers.add("Катя: Приветанне");
		answers.add("Катя: Уже все готово, не знаю, что делать со свободным временем. Уже все дела переделала "
				+ "и пластинки переслушала.");
		answers.add("Катя: Так это же твоя. Сам мне вчера вечером отдал.");
		answers.add("Катя: Да, есть. Отдам, если как-нибудь развлечешь меня.");
		questions.add("1. Как диплом?");
		questions.add("2. Чья гитара?");
		questions.add("3. А у тебя есть флешка?");
		message.setMessage("Катя: Спасибо за пластинку. Давно ее искала! Держи флешку");
	}
	...
	public void getItem(Items item) {//получение предмета
		if(item.getName() == "itemDisc") {//если нужный предмет
			playGuitarMusic();
			talkAnimation.reset();
			getItem = true;
			...
			message.setMessage("Катя: Спасибо за пластинку. Давно ее искала! Держи флешку");//установить текст сообщения
			...
			playGuitarMusic();
			talkAnimation.reset();
			message.setMessage("Катя: Мне это не нужно...");//установить сообщение
			showMessage = true;//показать сообщение
