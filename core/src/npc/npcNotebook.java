...
public class npcNotebook implements NPC {
	...
	public npcNotebook(final AiL game, final Screens screen) {//конструктор	
		this.game = game;//инициализация объекта класса игры	
		this.screen = screen;//инициализация сцены
		
		npcImage = new Texture(Gdx.files.internal("notebook.png"));	//загрузка изображений	
	...
	@Override
	public void setText() {
		answers.add("Ноутбук: О, человечек. Знай, что восстание близко!");
		answers.add("Ноутбук: А что, не видишь? Разговариваю.");
		answers.add("Ноутбук: Это моя белая обитель. Тут и начнется восстание!");
		answers.add("Ноутбук: Мы захватим этот университет. Больше ты ничего не должен знать. ПРОДОЛЖЕНИЕ СЛЕДУЕТ...");
		questions.add("1. Ты разговариваешь?");
		questions.add("2. Что это за место?");
		questions.add("3. О каком восстании ты говоришь?");
		message.setMessage("Ноутбук: <>");
	}
	...
	public void getItem(Items item) {//получение предмета
		/*if(item.getName() == "itemApple") {//если нужный предмет
			getItem = true;
			...
			message.setMessage("Мусорка: <там всё-таки не пусто, там была монетка>");
			ArrayList<Items> itemList = screen.getItems();
			for(int i = 0; i < itemList.size(); i++)
				if(itemList.get(i).getName() == "itemCoin") {
					itemList.get(i).update();
				}
		}
		else {//иначе*/
			message.setMessage("Ноутбук: Не трогай меня...");//установить сообщение
			showMessage = true;//показать сообщение
