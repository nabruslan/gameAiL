...
public class npcYanina implements NPC {
	...
	public npcYanina(final AiL game, final Screens screen) {//конструктор	
		this.game = game;//инициализация объекта класса игры	
		this.screen = screen;//инициализация сцены
		
		npcImage = new Texture(Gdx.files.internal("Yanina.png"));	//загрузка изображений	
		talkAnimation = new Animator(2, 2, "yaninaAnimation.png", game);
	...
	@Override
	public void setText() {
		answers.add("Янина: Приветик");
		answers.add("Янина: Все отлично, только жутко устала. Много мероприятий впереди.");
		answers.add("Янина: Это секрет... Назови пароль и узнаешь.");
		answers.add("Янина: Я недавно разговаривала с Дианой. Она же на скрипке играет. Говорит, что у нее творческий"
				+ "кризис. Как я её понимаю.");
		questions.add("1. Как работа?");
		questions.add("2. Что ты тут делаешь?");
		questions.add("3. Что расскажешь?");
		message.setMessage("Янина: Пароль верный! Заходи в эту дверь.");
	}
	...
	public void getItem(Items item) {//получение предмета
		if(item.getName() == "itemPassword") {//если нужный предмет
			talkAnimation.reset();
			getItem = true;
			...
			message.setMessage("Янина: Пароль верный! Заходи в эту дверь.");//установить текст сообщения
		}
		else {//иначе
			talkAnimation.reset();
			message.setMessage("Янина: Это мне не нужно...");//установить сообщение
			showMessage = true;//показать сообщение
